    ...
    def __init__(self, config):
        ...
        if isinstance(config,  dict):            
            ...
            for k in dir(config):
                if '__' not in k:
                    if k in paras.keys():
                        if paras[k] != getattr(config, k):
                            paras[k] = getattr(config, k)
                    else:
                        paras[k] = getattr(config, k)
...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, disturb=False):
        ...
        if disturb:
            ...
            g_norm_pos = self.position_embeddings.weight.grad.norm()
            g_norm_tok = self.token_type_embeddings.weight.grad.norm()
            
            g_emb = - (self.word_embeddings.weight.grad * epsilon / g_norm_emb)[input_ids]  
            g_pos = - (self.position_embeddings.weight.grad * epsilon / g_norm_pos)[position_ids]  
            g_tok = - (self.token_type_embeddings.weight.grad * epsilon / g_norm_pos)[token_type_ids]  
            
            #if input_ids[0][108]==102:
            if input_ids.shape[1] > 100:
                #pdb.set_trace()
                mask_mat = torch.ones_like(words_embeddings)
                #mask_mat[:,20:100,:] = 0
                mask_mat = mask_mat.to(words_embeddings.device)

                g_emb = g_emb * mask_mat 
                g_pos = g_pos * mask_mat 
                g_tok = g_tok * mask_mat 
                g_emb = g_emb / g_emb.norm()
                g_pos = g_pos / g_pos.norm()
                g_emb = g_emb / g_tok.norm()
            
            words_embeddings = words_embeddings + g_emb
            #position_embeddings = position_embeddings + g_pos
            #token_type_embeddings = token_type_embeddings + g_tok
...
class BertCrossAttention(nn.Module):
    ...
    def forward(self, graph_vectors, word_vectors, attention_mask=None, head_mask=None, drop_first_token=True, sent_ind=None):
        '''
        drop_first_token: if the [cls] token should be droped. For GraphExtractor, should. Whereas for GatResMerger, should not.
        The attention operation should be conducted within each sentence and without the [cls] token for the first sentence.
        Here below is a conpromising method, with graph vectors attented to all word_vectors (context vectors). 
        Which will impact modules involve with `BertCrossAttention` module, alias, `GraphExtractor` and `GATResMerger` 
        In future, literally sentence-wise attention could be achieved by introducing a mask upon attention score tensor using sentence_ind. 
        '''
        
        if drop_first_token:
            word_vectors = word_vectors[:, 1:, ] # drop the [cls] token
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        for key in state_dict.keys():
            ...
            if 'layer' in key:
                new_key = key.replace('layer', 'bert_layer')
            if new_key:
                old_keys.append(key)
...
class BertModel(BertPreTrainedModel):
    ...
    ```
    """
    def __init__(self, config, output_attentions=False, keep_multihead_output=False, pool=True):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, head_mask=None, disturb=False):
        ...
        embedding_output = self.embeddings(input_ids, token_type_ids, disturb=disturb)
        #pdb.set_trace()
...
class BertForNextSentencePrediction(BertPreTrainedModel):
    ...
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        ...
        try:
            ...
            print('is pretraining: ',config.is_pretrain)
        except:
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, next_sentence_label=None, head_mask=None, disturb=False):
        outputs = self.bert(input_ids, token_type_ids, attention_mask,
                                     output_all_encoded_layers=False,
                                     head_mask=head_mask, disturb=disturb)
        if self.output_attentions:
